...
export const getServerSideProps = async () => {
  const query = `*[_type == "post" &&  "Tools" in categories[]->title] {
    _id,  
    title,
    author -> {
      name,
...
export default function salaries({posts}) {
    ...
    return (
        ...
        <NextSeo
   title="Tools | eComPortal"
   description="Reccommendations For The Top Shopify Tools & Apps For Your Store   " 
   openGraph={{
 ...
 />
   <div className="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
     <div className="text-center sm:flex sm:items-end sm:space-x-16 sm:text-left">
       ...
       Reccommendations For The Top Shopify Tools & Apps For Your Store 
       </h2>
     ...
     <div className="grid grid-cols-1 gap-6 px-8 mt-12 sm:mt-16 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:px-0">
      {posts.map((post) => {
        return (
       <Link key={post._id} passHref legacyBehavior href={`/tools/post/${post.slug.current}`} >
<a>
            <div
