...
export const getServerSideProps = async () => {
  const query = `*[_type == "post" &&  "Building A Brand" in categories[]->title] {
    _id,  
    title,
    author -> {
      name,
...
export default function salaries({posts}) {
    ...
    return (
        ...
        <NextSeo
   title="Building a Brand | eComPortal"
   description="Learn the Fundamentals To Building A Succesful eCommerce Brand   " 
   openGraph={{
 ...
 />
   <div className="px-4 mx-auto font-Studio6 sm:px-6 lg:px-8 max-w-7xl">
     ...
     <div className="grid grid-cols-1 gap-6 px-8 mt-12 sm:mt-16 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:px-0">
      {posts.map((post) => {
        return (
       <Link key={post._id} passHref legacyBehavior href={`/building-a-brand/post/${post.slug.current}`} >
<a>
            <div
