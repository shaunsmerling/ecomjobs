...
export const getServerSideProps = async () => {
  const query = `*[_type == "post" &&  "Getting Hired" in categories[]->title] {
    _id,  
    title,
    author -> {
      name,
...
export default function salaries({posts}) {
    ...
    return (
        ...
        <NextSeo
   title="Getting Hired | eComPortal"
   description="A Guide To Getting Hired For Common Roles in eCommerce brands   " 
   openGraph={{
 ...
 />
   <div className="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
     <div className="text-center sm:flex sm:items-end sm:space-x-16 sm:text-left">
       ...
       A Guide To Getting Hired For Common Roles in eCommerce Brands 
       </h2>
     ...
     <div className="grid grid-cols-1 gap-6 px-8 mt-12 sm:mt-16 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 sm:px-0">
      {posts.map((post) => {
        return (
       <Link key={post._id} passHref legacyBehavior href={`/getting-hired/post/${post.slug.current}`} >
<a>
            <div
